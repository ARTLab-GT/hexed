...
int main()
{
  ...
  // Solution setup
  cartdg::Solver solver (3, row_size, 1.);
  ...
  cartdg::Isentropic_vortex vortex (std::vector<double> {100., 0., 0., 1.225, 101325/0.4});
  vortex.center0 = 0.5; vortex.center1 = 0.5;
  solver.initialize(vortex);

  // Let's go!
  #if CARTDG_USE_TECPLOT
  solver.visualize_field_tecplot(cartdg::State_variables(), "demo3_time_0");
  #endif
  #if CARTDG_USE_OTTER
  {
    otter::plot plt;
