...
class Basis
{
  ...
  Mat<> nodes() const; //!< \brief Get vector of nodes
  ...
  //! \brief Differentiation matrix.
  //! \details `diff_mat()(i, j)` is the derivative of interpolating polynomial `j` evaluated at `node(i)`.
  ...
  /*! \brief `orthogonal(degree)(i)` is the `degree`th-degree
   * [Legendre polynomial](https://en.wikipedia.org/wiki/Legendre_polynomials) evaluated at `node(i)`.
   ...
   * \details Current implementation is not very performance-optimized.
