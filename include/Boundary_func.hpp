...
class Boundary_func : virtual public Output_data
{
  public:
  /*!
   * \param face A `Boundary_face` object representing a face of an element that is on a boundary.
   * \param i_fqpoint %Index of the face quadrature point (\f$ \in [0, r^{n_{dim} - 1})\f$ where \f$r\f$ is row size).
   * \param time Flow time.
   */
  virtual std::vector<double> operator()(Boundary_face& face, int i_fqpoint, double time) const = 0;
};
 ...
 * \attention The stress is obtained by reading the flux storage of the `Boundary_face` object,
...
class Viscous_stress : public Boundary_func
{
  ...
  std::vector<double> operator()(Boundary_face&, int i_fqpoint, double time) const override;
};
