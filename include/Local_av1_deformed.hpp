...
class Local_av1_deformed : public Kernel<Deformed_element&>
{
  ...
  public:
  Local_av1_deformed(const Basis& basis, double d_time, bool use_av_coef = true, bool scalar_only = false, int tss_power = 1) :
    derivative{basis},
    write_face{basis},
    dt{d_time},
    ...
    for (int i_elem = 0; i_elem < elements.size(); ++i_elem)
    {
      ...
      for (int i_var = start; i_var < n + start; ++i_var) {
        for (int i_qpoint = 0; i_qpoint < n_qpoint; ++i_qpoint) {
          ...
          state[i_dof] += time_rate[i_var][i_qpoint]*d_t_by_d_pos/det[i_qpoint]*custom_math::pow(tss[i_qpoint], tss_pow);
        }
      }
