...
class Stopwatch {
  ...
  /*! \brief time currently on the stopwatch
   * \details Specifically, the total elapsed time between `start()`s and `stop()`s since the last `reset()`,
   * plus time since last `start()` if `running()`.
   */
  double time() const;
