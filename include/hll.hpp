  ...
  for (int i_qpoint = 0; i_qpoint < n_face_qpoint; ++i_qpoint)
  {
    ...
    for (int i_side = 0; i_side < 2; ++i_side) {
      ...
      for (int j_dim = 0; j_dim < n_var - 2; ++j_dim) {
        veloc += READ(j_dim)/READ(n_var - 2)*nrml[j_dim];
        pres += READ(j_dim)*READ(j_dim)/READ(n_var - 2);
      }
      pres = (heat_rat - 1.)*(READ(n_var - 1) - 0.5*pres);
      for (int j_dim = 0; j_dim < n_var - 2; ++j_dim) {
        FLUX(j_dim) = READ(j_dim)*veloc;
        FLUX(j_dim) += pres*nrml[j_dim];
      }
      FLUX(n_var - 2) = READ(n_var - 2)*veloc;
      FLUX(n_var - 1) = (READ(n_var - 1) + pres)*veloc;
      velocity[i_side] = veloc;
      sound_speed[i_side] = std::sqrt(heat_rat*pres/READ(n_var - 2))*nrml_mag;
      #undef FLUX
      #undef READ
    }
    double wave_speed [2];
