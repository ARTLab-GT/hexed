...
/*! \brief Base class for a "kernel".
 * "Kernel" is a word which here means a callable object which accepts dimensionality
...
class Kernel {
  ...
  virtual U operator()(Sequence<T>& sequence, Stopwatch_tree& category_tree, std::string name) {
    auto& specific_tree {category_tree[name]};
...
//! \brief declares a pointer type which can point to any template instantiation.
//! \brief must be defined by each kernel class.
template <template<int, int> typename kernel>
using ptr_t = typename kernel<1, 2>::ptr_t;

/*! \brief A recursive template to create the desired kernel instantiations.
 * \details You can think of it as a compile-time linked list,
 * where each template contains another template with one less row size.
 * When the row size is 1, it wraps around to `config::max_row_size`
 * but with one less dimension via specialization below.
 * The second specialization below terminates the recursion when both the row size and the dimension are 1.
 */
...
//! \brief specialization to make the recursion wrap around to `config::max_row_size` of one less dimension
...
/*! \brief Gets a `std::unique_ptr` to a `base_t` of `kernel`.
 * \details The underlying kernel will be instantiated with `n_dim` and `row_size` provided as template arguments.
 * Arguments must satisfy `3 >= n_dim > 0` and `config::max_row_size >= row_size > 1`.
 */
