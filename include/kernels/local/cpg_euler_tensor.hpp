        ...
        for (int i_inner = 0; i_inner < stride; ++i_inner)
        {
          ...
          for (int i_var = 0; i_var < n_var; ++i_var)
          {
            for (int i_qpoint = 0; i_qpoint < row_size; ++i_qpoint)
            {
              ...
              {
                row_w[i_var*row_size + i_qpoint]
                += diff_mat[i_qpoint*row_size + j_qpoint]*(  flux[i_var*row_size + j_qpoint]
                                                           + flux[i_var*row_size + j_qpoint]);
              }
              row_w[i_var*row_size + i_qpoint] *= cfl;
            }
          }
