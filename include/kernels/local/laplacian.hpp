#ifndef CARTDG_LAPLACIAN_HPP_
#define CARTDG_LAPLACIAN_HPP_
...
void laplacian(std::vector<int>& elem_inds, double* data, int i_axis,
  ...
  for (int i_elem : elem_inds)
  {
    for (int i_outer = 0; i_outer < n_rows; ++i_outer)
    {
      for (int i_inner = 0; i_inner < stride; ++i_inner)
      {
        ...
        {
          row_r(i_qpoint) = data[i_elem*n_qpoint + i_outer*stride*row_size + i_inner + i_qpoint*stride];
        }
        Eigen::Matrix<double, row_size, 1> row_w = diff_mat*row_r;
        for (int i_qpoint = 0; i_qpoint < row_size; ++i_qpoint)
        {
          data[i_elem*n_qpoint + i_outer*stride*row_size + i_inner + i_qpoint*stride] = row_w(i_qpoint);
        }
      }
    }
  }
