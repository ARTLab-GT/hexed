...
void average_neighbor(double** connections_r, double** connections_w, int* starts, int* n_connections, Eigen::VectorXd weights_1d, double d_t_by_d_pos)
{
  ...
  for (int stride=n_face_qpoint, i_axis=0; stride > 0; stride /= row_size, ++i_axis)
  {
    ...
    {
      const int con_offset = starts[i_axis] + 2*i_con;
