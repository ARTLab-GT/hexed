...
class Advection
{
  ...
  Mat<1> flux_num(Mat<n_var, 2> face_vars, Mat<n_dim> normal) const
  {
    ...
    return face_flux*Mat<2>{.5, .5} + std::max(char_speed(face_vars(all, 0)), char_speed(face_vars(all, 1)))*face_state*Mat<2>{.5, -.5}*normal.norm();
  }

  double char_speed(Mat<n_var> state) const
  {
    return std::max(state(Eigen::seqN(0, n_dim)).norm(), 1.);
  }
};
