...
void av_flux(elem_vec& elements, Basis& basis, Kernel_settings& settings)
{
  ...
  {
    double* derivative = elements[i_elem]->derivative();
    double* viscosity = elements[i_elem]->viscosity();
    for (int i_qpoint = 0; i_qpoint < n_qpoint; ++i_qpoint)
    {
      double qpoint_visc = 0.;
      for (int i_visc = 0; i_visc < n_visc; ++i_visc)
      {
        qpoint_visc += viscosity[i_visc]*interp[i_visc][i_qpoint];
      }
      derivative[i_qpoint] *= qpoint_visc*d_t;
    }
  }
}
