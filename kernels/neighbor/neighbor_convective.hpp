...
void neighbor_convective(elem_cons connections, Basis& basis, Kernel_settings& settings)
{
  ...
  for (unsigned stride=n_face_qpoint, i_axis=0; stride > 0; stride /= row_size, ++i_axis)
  {
    #pragma omp parallel for
    for (unsigned i_con = 0; i_con < connections[i_axis].size()/2; ++i_con)
    {
      double face_r [2*face_size];
      double face_w [2*face_size];
      Element* elem0 = connections[i_axis][2*i_con];
      Element* elem1 = connections[i_axis][2*i_con + 1];
      ...
      hll_cpg_euler<n_var - 2, n_face_qpoint>(face_r, face_w, mult, i_axis, heat_rat);
