...
void Accessible_mesh::_fit_surface() {
    ...
    if (!edge->glued()) {
      ...
      if (!edge->snapping_problem) {
        for (auto elem : edge->dependent_elements()) {
          ...
          face->reset();
          if (!elem->acceptable_quality()) edge->snapping_problem = true;
        }
      }
      if (edge->snapping_problem) {
        edge->reset();
        for (auto elem : edge->dependent_elements()) elem->boundary_face_3d()->reset();
      }
    }
  }
  CHECK_QUALITY
  ...
  for (auto& face : faces_3d) {
    plain_snap(face);
    if (!face.element()->acceptable_quality()) {
      face.snapping_problem = true;
      face.reset();
    }
    for (int i_edge = 0; i_edge < 4; ++i_edge) {
    ...
    if (elem.tree) {
      if (elem.record == 1) {
        ...
        for (Tree* child : elem.tree->children()) {
          add_elem(is_deformed, *child, 0).record = 0;
        }
      }
    }
  }
...
void Accessible_mesh::deform() {
  ...
  for (bool is_deformed : {0, 1}) {
    ...
    for (int i_elem = 0; i_elem < sz; ++i_elem) {
      ...
      if (elem.record == 3) {
        add_elem(!is_deformed, *elem.tree, elem.aniso_ref_level()).record = 0;
        elem.record = 2;
        elem.tree.unpair();
      }
    }
        ...
        for (int i_elem = 0; i_elem < n_orig[is_deformed]; ++i_elem) {
          ...
          if (elem.record == -1 && !elem.is_extruded()) {
            ...
            if (unref) {
              ...
              parent->unrefine();
              add_elem(is_def, *parent, 0).record = 0;
            }
          }
        }
      }
