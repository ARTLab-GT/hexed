...
Stream_printer::Stream_printer(std::ostream& stream, bool use_escape_codes, Format emph_format)
: _stream{stream}, _format_code(""), _reset_code(""), _replace_code("\n")
{
  if (use_escape_codes) {
...
void Stream_printer::operator()(std::string message, bool emph, bool replace) {
  #pragma omp critical
  {
    if (replace) _stream << _replace_code;
...
Compound_printer printers::info(std::vector<std::shared_ptr<Printer>> {
  std::make_shared<Stream_printer>(std::cout, true, Stream_printer::Format {.type = Stream_printer::bold,
                                                                            .color = Stream_printer::green})
});

Compound_printer printers::warn(std::vector<std::shared_ptr<Printer>> {
  std::make_shared<Stream_printer>(std::cerr, true, Stream_printer::Format {.color = Stream_printer::yellow,
                                                                            .light = true})
});
