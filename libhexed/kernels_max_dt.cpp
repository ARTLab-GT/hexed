...
#define COMPUTE_MAX_DT(Pde_templ, ...) { \
  return std::min((*kernel_factory<Spatial<Pde_templ, false>::Max_dt>(mesh.n_dim, mesh.row_size, mesh.basis, local_time, opts.use_filter, msc, msd, limit_threshold, mesh.n_var __VA_OPT__(,) __VA_ARGS__))(mesh.car_elems, opts.sw_car, "compute time step"), \
                  (*kernel_factory<Spatial<Pde_templ,  true>::Max_dt>(mesh.n_dim, mesh.row_size, mesh.basis, local_time, opts.use_filter, msc, msd, limit_threshold, mesh.n_var __VA_OPT__(,) __VA_ARGS__))(mesh.def_elems, opts.sw_def, "compute time step")); \
}
...
double max_dt_euler(Kernel_mesh mesh, Kernel_options opts, double msc, double msd, bool local_time) {
  double limit_threshold = -1.;
  COMPUTE_MAX_DT(euler::Pde)
}

double max_dt_navier_stokes(Kernel_mesh mesh, Kernel_options opts, double msc, double msd, double limit_threshold, bool local_time,
