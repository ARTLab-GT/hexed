...
isothermal = pd.read_csv("hexed_out_isothermal1/surface_data.csv")
isothermal.sort_values("angle", inplace = True)
isothermal = isothermal.loc[isothermal["angle"] < max_angle*np.pi/180]
adiabatic = pd.read_csv("hexed_out_adiabatic1/surface_data.csv")
...
plt.plot(angle, nusselt, label = r"simulation ($k_\infty$ based on const Pr)")
nusselt = np.array(isothermal["heat_flux"])/(np.array(adiabatic["temperature"]) - 329.631)*2/0.00776826
plt.plot(angle, nusselt, label = "simulation ($k_\infty$ based on best estimate)")
