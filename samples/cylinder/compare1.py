...
for i in [1, 2]:
    ...
    #plt.scatter(angle*180/np.pi, data[:, 1]/data[0, 1], label = f"Couchman, grid {i}")
    plt.scatter(angle*180/np.pi, data[:, 1], label = f"Couchman, grid {i}")
...
plt.plot(sim["angle"]*180/np.pi, sim["heat_flux"])
plt.grid(True)
plt.legend()
