...
unstab_grad = np.zeros((n_elem*row_size, n_elem*row_size))
...
for i_elem in range(n_elem):
    for i_row in range(row_size):
        ...
        for i_col in range(row_size):
            ...
            unstab_grad[i_elem*row_size + i_row, i_elem*row_size + i_col] += basis.derivative(i_row, i_col)
            for i_side in [0, 1]:
                for j_side in [0, 1]:
                    correction = 0.5*(2*j_side - 1)*basis.interpolate(i_col, 1-j_side)*basis.interpolate(i_row, 1-i_side)/weights[i_row]
                    unstab_grad[((i_elem+i_side)%n_elem)*row_size + i_row, ((i_elem+j_side)%n_elem)*row_size + i_col] += correction
diffusion = unstab_grad@unstab_grad
...
mc, step_mat = max_cfl(euler, diffusion)
eigvals, eigvecs = np.linalg.eig(step_mat)
plt.scatter(np.real(eigvals), np.imag(eigvals), label=f"Diffusion Fwd Euler, CFL = {mc:.2g}")
