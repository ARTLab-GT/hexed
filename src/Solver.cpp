...
void Solver::update(double stability_ratio)
{
  ...
  for (int i = 0; i < 2; ++i) {
    ...
    if (use_art_visc) {
      ...
      (*kernel_factory<Spatial<Element         , pde::Navier_stokes>::Local>(nd, rs, basis, dt, i))(acc_mesh.cartesian().elements(), sw_car, "local");
      (*kernel_factory<Spatial<Deformed_element, pde::Navier_stokes>::Local>(nd, rs, basis, dt, i))(acc_mesh.deformed ().elements(), sw_def, "local");
      ...
      (*kernel_factory<Spatial<Element         , pde::Navier_stokes>::Reconcile_ldg_flux>(nd, rs, basis, dt, i))(acc_mesh.cartesian().elements(), sw_car, "reconcile LDG flux");
      (*kernel_factory<Spatial<Deformed_element, pde::Navier_stokes>::Reconcile_ldg_flux>(nd, rs, basis, dt, i))(acc_mesh.deformed ().elements(), sw_def, "reconcile LDG flux");
    } else {
      ...
      (*kernel_factory<Restrict_refined>(nd, rs, basis))(acc_mesh.refined_faces(), stopwatch.children.at("prolong/restrict"));
      (*kernel_factory<Spatial<Element         , pde::Euler>::Local>(nd, rs, basis, dt, i))(acc_mesh.cartesian().elements(), sw_car, "local");
      (*kernel_factory<Spatial<Deformed_element, pde::Euler>::Local>(nd, rs, basis, dt, i))(acc_mesh.deformed ().elements(), sw_def, "local");
    }
