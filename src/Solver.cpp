...
void Solver::snap_faces()
{
      ...
      if (bc_sn == acc_mesh.surface_bc_sn()) {
        ...
        if (node_adj) {
          ...
          for (int i_sign = 0; i_sign < 2; ++i_sign) {
            for (int i_dim = 0; i_dim < nd - 1; ++i_dim) {
              int vol_dim = i_dim + (i_dim >= con.i_dim());
              int face_dim = con.i_dim() > nd - con.i_dim() - vol_dim;
              for (int j_dim = 0; j_dim < nd; ++j_dim) {
                Eigen::Map<Mat<>> face_data(con.element().faces[2*vol_dim + i_sign] + (2*params.n_var + j_dim)*nfq, nfq);
                faces[i_sign](all, i_dim*nd + j_dim) = math::dimension_matvec(basis.boundary()(con.inside_face_sign(), all), face_data, face_dim);
              }
            }
          }
          for (int i_dim = 0; i_dim < nd - 1; ++i_dim) {
    ...
    for (int i_con = 0; i_con < bc_cons.size(); ++i_con) {
      ...
      if (bc_sn == acc_mesh.surface_bc_sn()) {
        ...
        if (node_adj) {
          ...
          for (Row_index ind(params.n_dim, params.row_size, con.i_dim()); ind; ++ind) {
            double interp = 1.;
            node_adj[ind.i_face_qpoint()] *= (1 - interp);
            node_adj[ind.i_face_qpoint()] += interp*math::sign(con.inside_face_sign())*state[ind.i_qpoint(0)];
          }
        }
      }
    }
  ...
  if (nd == 3) {
    (*kernel_factory<Restrict_refined>(params.n_dim, params.row_size, basis, false, true))(acc_mesh.refined_faces());
    (*kernel_factory<Prolong_refined>(params.n_dim, params.row_size, basis, false, true))(acc_mesh.refined_faces());
    auto& cons = acc_mesh.deformed().face_connections();
    #pragma omp parallel for
    for (int i_con = 0; i_con < cons.size(); ++i_con) {
      double* state = cons[i_con].state();
      for (int i_qpoint = 0; i_qpoint < nfq; ++i_qpoint) {
        for (int i_side = 0; i_side < 2; ++i_side) {
          state[i_side*nfq*params.n_var + i_qpoint] = state[(i_side + 2)*nfq*params.n_var + i_qpoint];
        }
      }
    }
...
bool Solver::fix_admissibility(double stability_ratio)
{
  ...
  for (iter = 0; iter < n_iters; ++iter) {
    ...
    (*kernel_factory<Spatial<Element         , pde::Fix_therm_admis>::Max_dt>(nd, rs, basis, true, false, dt, dt))(acc_mesh.cartesian().elements(), stopwatch.children.at("fix admis.").children.at("cartesian"), "compute time step");
    (*kernel_factory<Spatial<Deformed_element, pde::Fix_therm_admis>::Max_dt>(nd, rs, basis, true, false, dt, dt))(acc_mesh.deformed ().elements(), stopwatch.children.at("fix admis.").children.at("deformed" ), "compute time step");
    dt = 1.;
    double linear = dt;
