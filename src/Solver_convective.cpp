...
#define COMPUTE_CONVECTION(Pde_templ, sw) \
  ...
  (*kernel_factory<Spatial<Element         , Pde_templ>::Local>(nd, rs, basis, dt, i_stage, is_local_time))(acc_mesh.cartesian().elements(), sw_car, "local"); \
  (*kernel_factory<Spatial<Deformed_element, Pde_templ>::Local>(nd, rs, basis, dt, i_stage, is_local_time))(acc_mesh.deformed ().elements(), sw_def, "local"); \
