...
#define COMPUTE_DIFFUSION(Pde_templ, sw, bc_fun) \
  ...
  (*kernel_factory<Spatial<Element         , Pde_templ>::Neighbor>(nd, rs, i_stage PDE_ARGS))(acc_mesh.cartesian().face_connections(), sw_car, "neighbor"); \
  (*kernel_factory<Spatial<Deformed_element, Pde_templ>::Neighbor>(nd, rs, i_stage PDE_ARGS))(acc_mesh.deformed ().face_connections(), sw_def, "neighbor"); \
  ...
  (*kernel_factory<Spatial<Element         , Pde_templ>::Local>(nd, rs, basis, dt, i_stage, compute_residual, allow_filter && _namespace->lookup<int>("use_filter").value() PDE_ARGS))(acc_mesh.cartesian().elements(), sw_car, "local"); \
  (*kernel_factory<Spatial<Deformed_element, Pde_templ>::Local>(nd, rs, basis, dt, i_stage, compute_residual, allow_filter && _namespace->lookup<int>("use_filter").value() PDE_ARGS))(acc_mesh.deformed ().elements(), sw_def, "local"); \
  if (!i_stage) { \
    ...
    (*kernel_factory<Spatial<Element         , Pde_templ>::Reconcile_ldg_flux>(nd, rs, basis, dt, i_stage, compute_residual, allow_filter && _namespace->lookup<int>("use_filter").value()))(acc_mesh.cartesian().elements(), sw_car, "reconcile LDG flux"); \
    (*kernel_factory<Spatial<Deformed_element, Pde_templ>::Reconcile_ldg_flux>(nd, rs, basis, dt, i_stage, compute_residual, allow_filter && _namespace->lookup<int>("use_filter").value()))(acc_mesh.deformed ().elements(), sw_def, "reconcile LDG flux"); \
  } \
