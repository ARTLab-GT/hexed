...
void Solver::euler(double dt, int i_stage)
{
  ...
  (*kernel_factory<Spatial<Element         , pde::Euler>::Local>(nd, rs, basis, dt, i_stage))(acc_mesh.cartesian().elements(), sw_car, "local");
  (*kernel_factory<Spatial<Deformed_element, pde::Euler>::Local>(nd, rs, basis, dt, i_stage))(acc_mesh.deformed ().elements(), sw_def, "local");
}
