...
void hexed::Solver::snap_faces()
{
  ...
  for (int i_con = 0; i_con < bc_cons.size(); ++i_con) {
    ...
    if (bc_sn == acc_mesh->surface_bc_sn()) {
      ...
      if (node_adj) {
        node_adj += (2*con.i_dim() + con.inside_face_sign())*nfq;
        Eigen::Map<Mat<>> adj0(node_adj, nfq);
        Mat<> adj1 = math::hypercube_matvec(to_lob, adj0);
        for (int i_qpoint = 0; i_qpoint < adj1.size(); ++i_qpoint) {
          if (((i_qpoint/lob.row_size)%(lob.row_size - 1)) || ((i_qpoint%lob.row_size)%(lob.row_size - 1))) {
            adj1(i_qpoint) += 0.03*(rand()%100)/100;
          }
        }
        adj0 = math::hypercube_matvec(from_lob, adj1);
      }
    }
  }
