...
TEST_CASE("Typed_boundary_connection")
{
  ...
  REQUIRE(element.faces[2] == tbc0.new_state(0, false));
  REQUIRE(tbc0.ghost_face(false) == tbc0.new_state(1, false));
  hexed::Typed_bound_connection<hexed::Element> tbc1 {element, 1,  true, 1};
  REQUIRE(element.faces[3] == tbc1.new_state(0, false));
  REQUIRE(tbc1.ghost_face(false) == tbc1.new_state(1, false));
  ...
  REQUIRE(tbc0.new_state(0, false) == tbc0.inside_face(false));
  REQUIRE(tbc0.new_state(1, false) == tbc0.ghost_face(false));
  REQUIRE(tbc1.new_state(0, false) == tbc1.inside_face(false));
  REQUIRE(tbc1.new_state(1, false) == tbc1.ghost_face(false));
...
TEST_CASE("Freestream")
{
  ...
  for (int i_qpoint = 0; i_qpoint < n_qpoint; ++i_qpoint) {
    ...
    tbc.inside_face(false)[1*n_qpoint + i_qpoint] = -10.;
    tbc.inside_face(false)[2*n_qpoint + i_qpoint] = 50.;
  ...
  for (int i_qpoint = 0; i_qpoint < n_qpoint; ++i_qpoint) {
    ...
    REQUIRE(tbc.ghost_face(false)[1*n_qpoint + i_qpoint] == Catch::Approx(30.));
    REQUIRE(tbc.ghost_face(false)[2*n_qpoint + i_qpoint] == Catch::Approx(-20.));
    REQUIRE(tbc.ghost_face(false)[3*n_qpoint + i_qpoint] == Catch::Approx(1.3));
    REQUIRE(tbc.ghost_face(false)[4*n_qpoint + i_qpoint] == Catch::Approx(1.2e5));
  }
}
  ...
  SECTION("supersonic inflow")
  {
    ...
    for (int i_qpoint = 0; i_qpoint < n_qpoint; ++i_qpoint) {
      ...
      for (int i_var = 0; i_var < 5; ++i_var) {
        tbc.inside_face(false)[i_var*n_qpoint + i_qpoint] = state(i_var);
      }
    ...
    for (int i_qpoint = 0; i_qpoint < n_qpoint; ++i_qpoint) {
      for (int i_var = 0; i_var < 5; ++i_var) {
        REQUIRE(tbc.ghost_face(false)[i_var*n_qpoint + i_qpoint] == Catch::Approx(i_qpoint ? fs[i_var] : inside_state(i_var)));
      }
    }
    ...
    for (int i_qpoint = 0; i_qpoint < n_qpoint; ++i_qpoint) {
      for (int i_var = 0; i_var < 5; ++i_var) {
        REQUIRE(tbc.ghost_face(true)[i_var*n_qpoint + i_qpoint] == Catch::Approx(i_qpoint ? 1. : 0.).scale(1.));
      }
    }
  }
}
...
TEST_CASE("Function_bc")
{
  ...
  for (int i_qpoint = 0; i_qpoint < n_qpoint; ++i_qpoint) {
    ...
    tbc.inside_face(false)[0*n_qpoint + i_qpoint] = 20.;
    tbc.inside_face(false)[1*n_qpoint + i_qpoint] = -10.;
    tbc.inside_face(false)[2*n_qpoint + i_qpoint] = 5.;
  ...
  REQUIRE(tbc.ghost_face(false)[2*n_qpoint + 0] == Catch::Approx(1.7));
  REQUIRE(tbc.ghost_face(false)[3*n_qpoint + 0] == Catch::Approx(1e5));
  ...
  SECTION("apply_state")
  {
    ...
    for (int i_qpoint = 0; i_qpoint < row_size; ++i_qpoint) {
      ...
      REQUIRE(  3*tbc.ghost_face(false)[0*row_size + i_qpoint]
              + 4*tbc.ghost_face(false)[1*row_size + i_qpoint] == Catch::Approx(7.));
      // require normal momentum flipped
      REQUIRE( -4*tbc.ghost_face(false)[0*row_size + i_qpoint]
  ...
  SECTION("apply_flux")
  {
    ...
    for (int i_qpoint = 0; i_qpoint < row_size; ++i_qpoint) {
      ...
      REQUIRE(  3*tbc.ghost_face(true)[0*row_size + i_qpoint]
              + 4*tbc.ghost_face(true)[1*row_size + i_qpoint] == Catch::Approx(-7.));
      // require normal momentum flux unchanged
      REQUIRE( -4*tbc.ghost_face(true)[0*row_size + i_qpoint]
              + 3*tbc.ghost_face(true)[1*row_size + i_qpoint] == Catch::Approx(-1.));
      // require scalar flux flipped
      REQUIRE(tbc.ghost_face(true)[2*row_size + i_qpoint] == Catch::Approx(-tbc.inside_face(true)[2*row_size + i_qpoint]));
      REQUIRE(tbc.ghost_face(true)[3*row_size + i_qpoint] == Catch::Approx(-tbc.inside_face(true)[3*row_size + i_qpoint]));
    }
  }
}
...
TEST_CASE("No_slip")
{
  ...
  SECTION("isothermal")
  {
    ...
    for (int i_qpoint = 0; i_qpoint < row_size; ++i_qpoint) {
      ...
      REQUIRE(std::sqrt(tbc.ghost_face(false)[3*row_size + i_qpoint]*tbc.inside_face(false)[3*row_size + i_qpoint]) == Catch::Approx(1e6*1.2));
    }
    for (int i_qpoint = 0; i_qpoint < row_size; ++i_qpoint) {
    ...
    for (int i_qpoint = 0; i_qpoint < row_size; ++i_qpoint) {
      for (int i_var = 0; i_var < 4; ++i_var) {
        REQUIRE(tbc.ghost_face(true)[i_var*row_size + i_qpoint] == Catch::Approx(flux[i_var]));
      }
    }
  }
  SECTION("specified flux")
  {
    ...
    for (int i_qpoint = 0; i_qpoint < row_size; ++i_qpoint) {
      ...
      REQUIRE(tbc.ghost_face(false)[3*row_size + i_qpoint] == Catch::Approx(state[3]));
    }
    for (int i_qpoint = 0; i_qpoint < row_size; ++i_qpoint) {
      for (int i_var = 0; i_var < 4; ++i_var) tbc.inside_face(true)[i_var*row_size + i_qpoint] = flux[i_var];
    }
  ...
  SECTION("specified emissivity")
  {
    ...
    for (int i_qpoint = 0; i_qpoint < row_size; ++i_qpoint) {
      for (int i_var = 0; i_var < 4; ++i_var) {
        tbc.inside_face(false)[i_var*row_size + i_qpoint] = state[i_var];
        tbc.inside_face(true)[i_var*row_size + i_qpoint] = flux[i_var];
      }
    }
...
TEST_CASE("Copy")
{
  ...
  for (int i_qpoint = 0; i_qpoint < n_qpoint; ++i_qpoint) {
    ...
    REQUIRE(tbc.ghost_face(false)[1*n_qpoint + i_qpoint] == Catch::Approx(-10.));
    REQUIRE(tbc.ghost_face(false)[2*n_qpoint + i_qpoint] == Catch::Approx(50.));
    REQUIRE(tbc.ghost_face(false)[3*n_qpoint + i_qpoint] == Catch::Approx(0.9));
    REQUIRE(tbc.ghost_face(false)[4*n_qpoint + i_qpoint] == Catch::Approx(1e4));
  }
}
