...
TEST_CASE("Boundary_connection") {
  ...
  REQUIRE(bc0.neighbor_connection().face(0).storage_params() == f0.storage_params());
  REQUIRE(bc0.neighbor_connection().face(0).i_dim() == 2);
  REQUIRE(bc0.neighbor_connection().face(0).sign() == 1);
  REQUIRE(bc0.neighbor_connection().face(0).is_deformed() == false);
