...
TEST_CASE("Ghost boundary conditions")
{
  ...
  SECTION("Axis 0")
  {
    Supersonic_inlet bc0 (def_grid, 0, false);
    def_grid.ghost_bound_conds.push_back(&bc0);
    bc0.add_element(0);
    for (int i_dim = 0; i_dim < 3; ++i_dim)
    {
      for (int i_qpoint = 0; i_qpoint < grid.n_qpoint; ++i_qpoint)
      {
        def_grid.deformed_element(0).jacobian()[i_dim*4*grid.n_qpoint + i_qpoint] = 3.;
      }
    }
        ...
        for (int i = 0; i < row_size; ++i)
        {
          for (int j = 0; j < row_size; ++j)
          {
            for (int k = 0; k < row_size; ++k)
            {
              ...
              #define  READ(i) g->element(i_elem).stage(0)[i_qpoint + (i)*grid.n_qpoint]
              #define WRITE(i) g->element(i_elem).stage(1)[i_qpoint + (i)*grid.n_qpoint]
    ...
    for (int i_elem = 0; i_elem < 27; ++i_elem)
    {
      ...
      for (int i_qpoint = 0; i_qpoint < grid.n_qpoint; ++i_qpoint)
      {
        #define WRITE(i) grid.element(i_elem).stage(1)[i_qpoint + (i)*grid.n_qpoint]
        if (std::abs(pos[i_qpoint] - 1.5) < 1e-15)
        {
    ...
    std::vector<double> pos = def_grid.get_pos(0);
    for (int i_qpoint = 0; i_qpoint < def_grid.n_qpoint; ++i_qpoint)
    {
      #define WRITE(i) def_grid.element(0).stage(1)[i_qpoint + (i)*grid.n_qpoint]
  ...
  SECTION("Axis 1")
  {
    ...
    for (int i_elem = 0; i_elem < 27; ++i_elem)
    {
      for (int i_qpoint = 0; i_qpoint < grid.n_qpoint; ++i_qpoint)
      {
        #define  READ(i) grid.element(i_elem).stage(0)[i_qpoint + (i)*grid.n_qpoint]
        #define WRITE(i) grid.element(i_elem).stage(1)[i_qpoint + (i)*grid.n_qpoint]
