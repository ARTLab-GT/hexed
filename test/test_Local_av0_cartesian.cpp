  ...
  for (int i = 0; i < row_size; ++i) {
    for (int j = 0; j < row_size; ++j) {
      for (int k = 0; k < row_size; ++k) {
        int inds [] {i, j, k};
        double correct = 0.;
        for (int i_dim = 0; i_dim < 3; ++i_dim) correct += linear(k)*scale[i_dim]*deriv_cubic(inds[i_dim]);
        correct += scale[2]*cubic(k);
        correct *= .3/.4*2; // .3 for time step, .4 for mesh size, 2 for ref level
        int i_qpoint = (i*row_size + j)*row_size + k;
