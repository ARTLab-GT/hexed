...
TEST_CASE("Mcs_deformed")
{
  ...
  REQUIRE((*cartdg::kernel_factory<cartdg::Mcs_deformed>(3, 4))(elem_view) == Approx(1e3/0.3));
  ...
  REQUIRE((*cartdg::kernel_factory<cartdg::Mcs_deformed>(3, 4))(elem_view) == Approx(2e3*0.95/0.3));

  for (int i_dim = 0; i_dim < 2; ++i_dim) {
    // scale 2 entries of jacobian at one quadrature point.
    elems[2]->jacobian()[(i_dim*3 + i_dim)*n_qpoint + 2] = 0.01;
  }
  REQUIRE((*cartdg::kernel_factory<cartdg::Mcs_deformed>(3, 4))(elem_view) == Approx(1e4/0.3));
}
