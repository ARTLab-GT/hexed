...
#include <kernels/local/cpg_euler_max_char_speed.hpp>
...
TEST_CASE("Max characteristic speed")
{
  SECTION("1D")
  {
    ...
    double mcs = cpg_euler_max_char_speed<3, 2>(read, 2, sp_heat_rat);
    REQUIRE(mcs == Approx(420));
  }
  SECTION("2D")
  {
    double read[4] {2.25, 24.5, 1.225, 101235/0.4 + 0.5*1.225*500};
    double mcs = cpg_euler_max_char_speed<4, 1>(read, 1, 1.4);
